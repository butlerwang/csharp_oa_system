<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="YiBanWork.aspx.cs" Inherits="OA.aspx.NWorkFlow.YiBanWork" %>
<html>
	<head>
...
<asp:Button ID="iButton2" runat="server" CssClass="btn btn-aqua" Text="导出" OnClick="iButton2_Click" />&nbsp;<button type="button" class="btn btn-yellow" onclick="javascript:window.history.go(-1)">返回</button></td>
        ...
        <table style="width: 100%">
            ...
            <td ><asp:GridView ID="GVData" CssClass="gridview_m"  runat="server" AllowPaging="True" AllowSorting="True" AutoGenerateColumns="False"
					...
					<asp:TemplateField HeaderText="工作名称"> <ItemTemplate> <asp:HyperLink ID="HyperLink1" runat="server" Font-Underline="False" ForeColor="green" NavigateUrl='<%# "NWorkToDoView.aspx?ID="+ DataBinder.Eval(Container.DataItem, "ID")%>'><%# DataBinder.Eval(Container.DataItem, "WorkName")%></asp:HyperLink>&nbsp;<asp:HyperLink ID="HyperLink21" runat="server" Font-Underline="False" ForeColor="green" ToolTip='<%# DataBinder.Eval(Container.DataItem, "ShenPiUserList")%>' NavigateUrl='<%# "DoWork.aspx?DoType=putong&ID="+ DataBinder.Eval(Container.DataItem, "ID") %>' Font-Bold="true" >[继续办理]</asp:HyperLink></ItemTemplate>   <ItemStyle HorizontalAlign="Left" />  </asp:TemplateField> 
