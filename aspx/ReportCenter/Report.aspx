<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Report.aspx.cs" Inherits="OA.aspx.ReportCenter.Report" %>
<html>
	<head>
...
<asp:Button ID="iButton2" runat="server" CssClass="btn btn-aqua" Text="导出" OnClick="iButton2_Click" />&nbsp;<button type="button" class="btn btn-yellow" onclick="javascript:window.history.go(-1)">返回</button></td>
        ...
        <table style="width: 100%">
            ...
            <td ><asp:GridView ID="GVData" CssClass="gridview_m"  runat="server" AllowPaging="True" AllowSorting="True" AutoGenerateColumns="False"
					...
					<asp:TemplateField HeaderText="报表名称"> <ItemTemplate> <asp:HyperLink ID="HyperLink1" runat="server" Font-Underline="False" ForeColor="green" NavigateUrl='<%# "ReportView.aspx?ID="+ DataBinder.Eval(Container.DataItem, "ID")%>'><%# DataBinder.Eval(Container.DataItem, "ReportName")%></asp:HyperLink> </ItemTemplate>   <ItemStyle HorizontalAlign="Left" />  </asp:TemplateField> 
					...
					<asp:TemplateField HeaderText="查看报表"> <ItemTemplate> [<asp:HyperLink ID="HyperLink10" runat="server" Font-Underline="False" ForeColor="green" NavigateUrl='<%# "ReportViewData.aspx?ID="+ DataBinder.Eval(Container.DataItem, "ID") %>' >查看报表</asp:HyperLink>] </ItemTemplate>   <ItemStyle HorizontalAlign="Center" />  </asp:TemplateField> 
